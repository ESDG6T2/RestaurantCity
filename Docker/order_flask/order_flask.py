...
class Order(db.Model):
    ...
    def json_with_items(self):
        ...
        output['orderItems']=[orderlist.json() for orderlist in OrderDetail.query.filter_by(orderId=self.orderId).all()] 
        return output
...
def add_order(orderId):
    ...
    for item in items:
        orderDetail = OrderDetail(data['orderId'], item['menuId'],item['quantity'])
        try:
